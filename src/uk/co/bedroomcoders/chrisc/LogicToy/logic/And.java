...
public class And extends LogicGate {
	...
	public void updateState(stateEvent se) {
		...
		if (tile.getIsInput(DIR.NORTH)==false) ns = true;
		if (tile.getIsInput(DIR.EAST )==false) es = true;
		if (tile.getIsInput(DIR.SOUTH)==false) ss = true;
		if (tile.getIsInput(DIR.WEST )==false) ws = true;
